---
date: '2025-12-09T22:55:34+09:00'
update: '2025-12-14T01:02:51+09:00'
draft: false
title: 'Uxntal'
tags:
  - Assembly
...
Uxntal은 Uxn CPU 위에서 돌아가는 {{%h primary7%}}ROM{{%/h%}}으로 컴파일 되기 위한 Assembly 언어이다.  

{{< callout >}}
Uxntal은 여느 프로그래밍 언어와 마찬가지로 상호의존적으로 엮여진 구조이기에, 순서대로 배우기는 어렵지만, 최대한 독립적인 개념부터 정리했다.
{{</ callout >}}

## {{%h primary7%}}Read Only Memory (ROM){{%/h %}}

{{%h primary7%}}ROM{{%/h %}}은 Uxntal로부터 어셈블된다. 이름이 **ROM**(**R**ead **O**nly **M**emory)이지만, Uxn에 로드되는 순간, **`STA` 등을 이용해 수정될 수 있다**.

Uxntal의 [opcode](#opcodes)를 포함한 대부분의 데이터는 그대로 byte로 변환된다.

Uxn은 기본적으로 {{%h primary7%}}ROM `0x0100`{{%/h %}} 부터 로드되고, {{%h primary7%}}`0x0100`{{%/h %}} 부터 작성된다. 하지만 원한다면 특정 주소부터 작성을 시작할 수 있다.

<div id=zero-page>
{{< callout warning >}}
{{%h primary7%}}`0x0100`{{%/h %}} 이하의 주소는 **zero-page**라고 불리고, Uxn/Varvara 시스템에 의해 사용되기에, 사용자가 작성하려 하면 에러가 나고, 실제 {{%h primary7%}}ROM{{%/h %}}도 {{%h primary7%}}`0x0100`{{%/h %}}부터 작성된다. 그렇기에 보통 라벨만 붙이고 사용한다.
...
## {{%h primary1%}}Random Access Memory (RAM){{%/h %}}

Uxn은 2개의 256bytes의 스택으로 구성된 {{%h primary1 %}}RAM{{%/h%}}을 제공한다.  
RAM은 기본으로 작업하는 공간인 {{%h primary1%}}WST{{%/h%}}(**W**orking **ST**ack)과 추가적으로 연산에 활용할 수 있는 {{%h primary1%}}RST{{%/h%}}(**R**eturn **ST**ack)으로 이루어져있다.  

**Memory:**
 - {{%h primary1%}}WST{{%/h%}}(Working Stack)
 - {{%h primary1%}}RST{{%/h%}}(Return Stack)

## Data Structure

데이터는 0\~255(2<sup>8</sup>-1) 범위의 정수인 {{%h primary1%}}byte{{%/h %}}, 0\~65535(2<sup>16</sup>-1) 범위의 정수인 {{%h primary1%}}short{{%/h %}}로 구분되지만, 그 차이는 스택에서 차지하는 양의 차이일 뿐이다.
Byte 데이터는 두 자리 hex( {{%h primary1%}}`0x00`{{%/h%}} )로, short 데이터는 네 자리 hex({{%h primary1%}}`0x0000`{{%/h %}})로 스택에 삽입할 수 있지만, 스택 내부에서의 구분은 없다.
...
**Data:**
 - {{%h primary1%}}Byte{{%/h %}}
 - {{%h primary1%}}Short{{%/h %}} = Byte &times; 2
...
라벨은 특정 {{%h primary7%}}ROM 주소{{%/h %}}에 이름을 붙이는 역할을 한다. 
...
**Constant**는 uxntal에 따로 변수의 문법이 없기 때문에, {{%h primary7%}}ROM{{%/h %}}에 데이터를 쌓아두고 그 {{%h primary7%}}주소{{%/h %}}를 참조하여 변수로 사용한다.
...
{{< callout >}}
사실 Uxntal {{%h primary7%}}ROM{{%/h %}}을 읽기 시작하는 지점인 {{%h primary7%}}`0x0100`{{%/h %}}에는 기본적으로 {{%h primary7%}}`@on-reset`{{%/h %}}이라는 vector가 할당되어있다.
{{</ callout >}}

### Padding


`|`를 이용해 특정 {{%h primary7%}}ROM 메모리{{%/h %}} 부터 작성을 시작할 수 있다. 하지만 Uxn은 {{%h primary7%}}ROM `0x0100`{{%/h %}} 부터 실행할 것이기에, 실행을 원한다면 {{%h primary7%}}`0x0100`{{%/h %}}에 코드를 작성해야 한다. 또한, 지나온 {{%h primary7%}}메모리{{%/h %}}에 *작성*할 수 없기 때문에 작은 순서로 작성해야한다. (라벨을 붙이는 것 만이라면 가능하다.)
...
`$`를 이용해 현재 {{%h primary7%}}ROM 주소{{%/h %}}에서 상대적으로 움직일 수 있다.

라벨과 함께 이용해서 {{%h primary7%}}ROM{{%/h %}}의 특정 위치에 구조체(struct)를 만들 수 있다.
...
스택에 데이터를 삽입하는 runes
 ...
 - `#`로 hex값을 {{%h primary1%}}스택{{%/h %}}에 삽입할 수 있다. [Literal (`LIT`)](#lit)

 - `"`로 아스키 문자열의 각 글자를 byte로 {{%h primary7%}}ROM{{%/h %}} 삽입할 수 있다.

    <div id=raw>{{< callout warning >}}
스택에 삽입하는 것이 아니다. 어셈블러가 `"`를 만나면, 뒤의 문자열을 아스키 hex로 변환해 **그대로** {{%h primary7%}}ROM{{%/h %}}에 삽입한다.
```tal
( test.rom )
"banana
```
...
Uxntal에는 {{%h primary7%}}ROM 주소{{%/h %}}를 다루는 6가지 runes가 존재한다. 각 rune은 라벨의 {{%h primary7%}}주소{{%/h %}}를 가져오는 역할을 한다.

**Literal\-** 은 실행 당시에 그 {{%h primary7%}}주소{{%/h %}}를 가져와서 {{%h primary1%}}스택{{%/h %}}에 삽입한다.
...
{{%details summary="Result & Analyze"%}}
...
```
 ...
 - (1) `0x0a`: [LIT2](#lit)의 opcode, 즉 다음에 오는 short를 stack에 삽입
 - (2) `0x0103`: `@label`의 ROM 주소

{{%/details%}}
{{%/ callout %}}

**Raw\-** 는 어셈블러 시점에 {{%h primary7%}}주소{{%/h %}}를 가져와서 [{{%h primary7%}}ROM{{%/h %}}에 삽입](#raw)한다.
...
```
 ...
 - (1) `0x0102`: `@label`의 ROM 주소

결과에서도 알 수 있듯, **Raw Labeling**은 어셈블리 단계에서 {{%h primary7%}}라벨{{%/h %}}의 {{%h primary7%}}ROM 주소{{%/h %}}를 삽입하기에, {{%h primary7%}}라벨의 주소(`0x0102`){{%/h %}}가 **Litral Labeling**({{%h primary7%}}`0x0103`{{%/h %}}) 보다 하나 앞으로 간 것을 볼 수 있다.  
{{%/details%}}
{{%/callout%}}
...
**PC**는 현재 opcode의 다음 {{%h primary7%}}ROM 주소{{%/h %}}가 되며, 각 opcode 실행 후, 2[^3]씩 증가한다.[^2]

[^3]: 2씩 증가하는 이유는 [opcode가 2 bytes를 차지](#opcode-byte)하기 때문이다.
...
`--` 앞의 각 아이템을 실행 전 {{%h primary1%}}스택{{%/h %}}의 상태, 뒤의 각 아이템을 실행 후 {{%h primary1%}}스택{{%/h %}}의 상태로 표기한다.
...
[RST에 영향을 끼치는 연산](#r-mode)이라면, `.` 뒤에 그 {{%h primary1%}}RST{{%/h %}}의 상황을 표기한다.
...
**Stash**(`STH`)는 {{%h primary1%}}스택(WST){{%/h %}}의 최상단 {{%h primary1%}}byte{{%/h %}}를 소모하여 {{%h primary1%}}RST{{%/h %}}으로 삽입한다.  
`STHr`은 {{%h primary1%}}RST{{%/h %}}의 최상단 {{%h primary1%}}byte{{%/h %}}를 소모하여 {{%h primary1%}}WST{{%/h %}}으로 삽입한다.
{{</ details >}}
{{</ callout >}}
...
<span id=opcode-byte>따라서, Uxntal의 opcode는 각 2 bytes를 차지한다.</span>
...
Immediate Opcode는 스택 최상단에서 데이터를 소모하여 연산하지 않고, opcode 이후에 {{%h primary7%}}ROM에 쓰여진 값{{%/h %}}을 ***즉시*** 가져와 사용하는 opcode들이다.  
...
{{%callout%}}
{{%details summary="Literal(`LIT`)"%}}
**Literal**(`LIT`)은 사실 `#`이외에도 쓰일 수 있는데, ROM에서 다음에 오는 데이터를 그대로 스택에 삽입하는 opcode이다.
```tal
( test.tal )
LIT "a
( RESULT
...
00000000: 8041                                     .A
    ...
    (1)---+  +--(2)
```
 - (1) `0x80`: `LIT`의 opcode, 즉 {{%h primary7%}}ROM{{%/h%}}의 다음 {{%h primary7%}}byte(`0x41`){{%/h%}}를 스택에 삽입한다.
 - (2) `0x41`: `A`의 ascii 값

> `LIT2`는 물론 {{%h primary7%}}ROM{{%/h %}}에서 두 byte를 소모한다.
{{%/details%}}
{{%/callout%}}
...
연산 시, {{%h primary1%}}스택{{%/h %}}에서 앞의 byte를 가져와 그 값에 따라 앞의 byte를 가져와 그 값에 따라 루틴으로 점프한다. 즉, 조건 점프는 byte를 **소모**한다.

`?` 이후 루틴이 올 경우, {{%h primary1%}}스택{{%/h %}} 최상위 byte를 소모하고, **`0x00`이 아니라면  루틴으로 점프**한다. `0x00`이라면 그대로 진행한다. (공식 문서에서는 **PC**를 2 증가시킨다고도 표현한다.)
...
<span id=anonymous>`?` 이후 `{`, `}` 으로 감싸진 부분</span>이 나온다면 **이전과 반대로 {{%h primary1%}}스택{{%/h %}} 최상위 byte가 `00`이라면** 내부를 실행한다.  
...
실행된 위치의 {{%h primary7%}}ROM 주소(PC){{%/h %}}를 저장하기에, 루틴을 실행하고 원래의 위치로 돌아올 수 있다는 점을 이용해 함수 같이 쓰인다. 그리고 돌아오게 만들 수 있기에, 내부 어느 시점에서 쓰인다고 하여 **서브루틴**이라고도 한다.
